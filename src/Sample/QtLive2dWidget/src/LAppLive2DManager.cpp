...
void LAppLive2DManager::StartMotionAndExpression(LAppModel* currentModel, std::string expressionName, std::string motion)
{
    csmString expressionCsmString(expressionName.c_str());
    const csmChar* motionCsmChar = motion.c_str();
    currentModel->StartRandomMotion(motionCsmChar, 2);
    currentModel->SetSpecificExpression(expressionCsmString);
}
