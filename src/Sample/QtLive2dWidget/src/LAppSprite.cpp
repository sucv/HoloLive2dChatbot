    ...
    : _rect()
{
    ...
    // 何番目のattribute変数か
    _positionLocation = OpenGLHelper::get()->glGetAttribLocation(programId, "position");
    _uvLocation      = OpenGLHelper::get()->glGetAttribLocation(programId, "uv");
    _textureLocation = OpenGLHelper::get()->glGetUniformLocation(programId, "texture");
    _colorLocation = OpenGLHelper::get()->glGetUniformLocation(programId, "baseColor");
...
void LAppSprite::Render() const
{
    ...
    // attribute属性を有効にする
    OpenGLHelper::get()->glEnableVertexAttribArray(_positionLocation);
    OpenGLHelper::get()->glEnableVertexAttribArray(_uvLocation);

    // uniform属性の登録
    OpenGLHelper::get()->glUniform1i(_textureLocation, 0);
    ...
    // attribute属性を登録
    OpenGLHelper::get()->glVertexAttribPointer(_positionLocation, 2, GL_FLOAT, false, 0, positionVertex);
    OpenGLHelper::get()->glVertexAttribPointer(_uvLocation, 2, GL_FLOAT, false, 0, uvVertex);

    OpenGLHelper::get()->glUniform4f(_colorLocation, _spriteColor[0], _spriteColor[1], _spriteColor[2], _spriteColor[3]);
